#!/usr/bin/env python3
"""
Transient 3D A–V eddy-current driver (volume-coil / bulk-current variant).

``A`` uses an H(curl) Nedelec space on the full mesh; ``V`` and the conducting
block use a submesh over rotor, coils, and PMs. Coils are driven by a prescribed
three-phase current density ``J_z`` (see ``load_mesh.CURRENT_MAP`` and
``utils.update_currents`` / forms).

Writes ``result.xdmf`` / ``result.h5`` (A, V, B fields) and, when enabled, VTX
``V.bp`` and ``J.bp`` on the conductor submesh and motor-region ``B.bp``.

Requires ``mesh.xdmf`` from ``mesh.py`` in this directory.

Run:
  cd src/3d_volume_coils
  python main.py
"""
...
def main():
    ...
    if config.write_results:
        ...
        # 4: legacy shaft tag; 5: rotor assembly (new meshes use 5 only)
        ...
        if motor_cells:
            ...
            B_motor_writer = {
                ...
                "function": B_motor,
                "writer": VTXWriter(
                    mesh_parent.comm, str(bp_dir / "B.bp"), B_motor
                ),
            }
